...
class Block::Iter : public Iterator {
 ...
 public:
  ...
  void Seek(const Slice& target) override {
	...
	// if(restart_index_ > 50 && run == 1 ){
		// left = 1;
	// cout << "SLR Active: " << slr_ << endl;
	// }else 
		
	if(restart_index_ > 50 && slr_ ){
		// cout << "slr_ seek: " << slr_ << endl;
		uint32_t find =  stoi(togot);
		// uint32_t dividendMeta=DividendMeta(data_,size_,1);
		// uint32_t divisorMeta=DivisorMeta(data_,size_,1);
		// uint32_t lowestMeta=LowestMeta(data_,size_,1);
		// uint32_t iteration = LowestMeta(data_,size_,1);
		uint32_t iteration;
		uint32_t lowestMeta=LowestMeta(data_,size_,1);
		int which_segment = 1;
		...
		for(int seg=1; seg <= segment_len; seg ++){
			// uint32_t next_lm=LowestMeta(data_,size_,seg + 1);
			iteration=LowestMeta(data_,size_,seg);
			
			// cout << "Find: " << find << " seg:" << seg << " iteration:" << iteration << endl; 
			// if(find >= lm){
				// which_segment=seg;
				// lowestMeta=lm;
			// }
			...
			// if(seg < segment_len){
				// iteration = LowestMeta(data_,size_,seg + 1);
			// }
			// }else if(seg == 1 && find < lm){
				// which_segment=seg;
				// lowestMeta=lm;
			// }	 
		}
		...
		// if(find < lowest_){
		// if(find < lowestMeta){
		// if(which_segment ==1 && find < lowestMeta){
    ...
    while (true) {
		// if(debug_){
			// cout << "Linear Search Loop " << lp << endl;
			// lp++;
		// }
