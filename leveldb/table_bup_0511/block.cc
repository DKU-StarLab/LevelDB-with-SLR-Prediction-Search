...
inline uint32_t Block::Dividend() const {
  assert(size_ >= sizeof(uint32_t));
  return DecodeFixed32(data_ + size_ - sizeof(uint32_t) - sizeof(uint32_t));
}

 //fill LR lowest subset
inline float Block::Lowest() const {
  assert(size_ >= sizeof(uint32_t));
  return DecodeFloat32(data_ + size_ - sizeof(uint32_t) - sizeof(uint32_t) - sizeof(uint32_t));
}
...
Block::Block(const BlockContents& contents)
  ...
  if (size_ < sizeof(uint32_t)) {
    ...
    if (NumRestarts() > max_restarts_allowed) {
      ...
      restart_offset_ = size_ - (3 + NumRestarts()) * sizeof(uint32_t);
...
class Block::Iter : public Iterator {
 ...
 public:
  ...
  Iter(const Comparator* comparator, const char* data, uint32_t restarts,uint32_t num_restarts, uint32_t dividend, float lowest)
  ...
  void Seek(const Slice& target) override {
	...
	if(restart_index_ > 50 && run > 0){
		float coef= dividend_ / (restart_index_-2);
		uint32_t cari =  stoi(togot);;
		...
		if(cari < lowest_){
			//lower than first restart point
			cout << "lower than first restart point " << "\n";
		}else{
			int atas = cari - lowest_;
			int prediksi = atas/coef;
			float prediksi2 = atas/coef;
			...
			// int cmp=CompareInt(mid_key, target);
			...
			if (cmp < 0) {
				...
				for(int b= prediksi+1 ; b <= max; b++){
					...
					cout << "CPR " << Compare(mid_key_in,target) << endl;
					if (Compare(mid_key_in, target) > 0) {
						// *index = static_cast<uint32_t>(b);
						cout << "		Index hpuskiri: " <<  b <<"\n\n";
						left = b;
						break;
					}
    ...
    while (true) {
		cout << "before !ParseNextKey()" << endl;
      if (!ParseNextKey()) {
		  cout << "!ParseNextKey()" << endl;
        return;
      }
	  cout << "after !ParseNextKey()" << endl;
...
Iterator* Block::NewIterator(const Comparator* comparator) {
  ...
  const uint32_t num_restarts = NumRestarts();
   ...
   //init LR dividend
  const uint32_t dividend = Dividend();
  //init LR lowest subset
  const float lowest = Lowest();
  ...
  if (num_restarts == 0) {
    ...
    return new Iter(comparator, data_, restart_offset_, num_restarts,dividend, lowest);
  }
}
